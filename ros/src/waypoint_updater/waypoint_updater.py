...
VERBOSE = False
DEVELOPER = False
ARBITRARY_WAYPOINT_NUMBERING = True
USE_TRAFFIC_LIGHTS = True
BRAKING_DISTANCE_START = 10   #Intended to be the distance before the light at which braking starts 
...
class WaypointUpdater(object):
    ...
    def find_and_publish_final_waypoints(self):
        ...
        traffic_light_index = 350  #just an example here
        if DEVELOPER:
            t1 = time.time()
        ...
        for index, waypoint in enumerate(self.base_waypoints):
            ...
            if traffic_distance < BRAKING_DISTANCE_START:
                ...
                waypoints[index].twist.twist.linear.x = self.current_velocity*0.9
        ...
        #This is somewhat abandoned code... for non-arbitrary waypoint numbering
        #    closest_point = -1;
        #    if self.older_point > 0:
        #        projected_point = int(2*self.old_point - self.older_point)
        #    else:
        #        projected_point = int(total_points*.5)
        #    while projected_point != closest_point:
        #        waypoint_distance = math.sqrt((self.current_pose.position.x-waypoint.pose.pose.position.x)**2+
        #                                  (self.current_pose.position.y-waypoint.pose.pose.position.y)**2)
